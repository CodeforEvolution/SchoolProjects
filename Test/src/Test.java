...
public class Test {
	...
	{
   	 	Canvas myVisual = new Canvas("Let's Count!!!");
   	 	int i = 0;
   	 	String currentNumber = "";
   	 	Point clickedWhere = new Point(100,100);
   	 	
        myVisual.setSize(600, 400);
        myVisual.fillBackground(Color.BLACK);
        myVisual.setInkColor(Color.GREEN);
        myVisual.setFontSize(70);
        myVisual.setVisible(true);
        
        System.out.println("Click Everywhere!");
        
        while (true)
        {
        	currentNumber = Integer.toString(i);
        	myVisual.drawString(currentNumber, clickedWhere.x, clickedWhere.y);
        	clickedWhere = myVisual.waitForClick();
        	i++;
        	myVisual.erase();
        	myVisual.fillBackground(Color.BLACK);
        }
